...
cy.get('.anomaly-widget-card__title').filter(':visible').should('have.length.greaterThan', 0).each(($title) => {
  ...
  cy.wrap($title)
    ...
    .then(($card) => {
      ...
      if (isHidden) {
        // 🌟 [신규 추가] 숨김 카드는 콘텐츠 검증 없이 "숨김" 태그만 확인하고 통과
...
//2.9.1.125_r35234 input은 aria-label 속성이 없고, placeholder 속성으로 변경
