...
// STEP: 보관 > 운영이력 보관
// ==========================================
cy.contains('button', '보관').should('be.visible').click({ force: true });
cy.wait(2000);
cy.log('---보관 - 운영이력 보관 서브메뉴 클릭 ---');
cy.get('.v-list__tile__title').filter(':contains("운영이력 보관")').filter(':visible').click({ force: true });
cy.wait(2000); 

cy.log('--- 화면 검증 시작 ---');

// 섹션 헤드라인 2종 확인 디버깅
//cy.get('.c-headline').then($els => {
//cy.log(`총 ${$els.length}개의 .c-headline 발견`);
//  $els.each((i, el) => cy.log(`[${i}] text: "${el.innerText}" / visible: ${Cypress.dom.isVisible(el)}`));
//});

// 섹션 헤드라인 
cy.get('.c-headline:visible').contains('운영이력 보관').should('be.visible');
cy.get('.c-headline:visible').contains('압축(최대 비율)').should('be.visible');

// [운영이력 보관 섹션]
cy.log('--- 운영이력 보관 섹션 ---');
cy.get('input[aria-label="활성/비활성"]').eq(0).should('exist');
cy.get('input[aria-label="보관일자(일)"]')
  .filter(':visible')
  .should('be.visible')
  .should('have.value', '1825');

// 안내 문구 확인
cy.contains('보관일자가 지난 운영 이력을 백업 작업 시 삭제합니다').should('be.visible');
cy.contains('이 설정은 DB 운영 이력 보관이며, 로그 파일 보관일수 설정과는 별개입니다').should('be.visible');

// [압축(최대 비율) 섹션]
cy.log('--- 압축(최대 비율) 섹션 ---');
cy.get('input[aria-label="활성/비활성"]').eq(1).should('exist');

// 압축 일자(일)
cy.get('input[aria-label="압축 일자(일)"]')
  .filter(':visible')
  .should('be.visible')
  .should('have.value', '180');

// 압축 보관 경로 (placeholder만 있고 값은 비어있는 상태)
cy.get('input[aria-label="압축 보관 경로"]')
  .filter(':visible')
  .should('be.visible')
  .should('have.attr', 'placeholder', '/home/logcatch/backup');

cy.contains('압축 일자가 지난 운영 이력을 일 단위로 최대 비율 압축(csv.gz)하여 백업 경로(work-history)에 보관합니다')
  .should('be.visible');
cy.contains('비우면 [보관>접속기록 보관]의 백업 경로 하위 work-history 에 저장합니다. 지정 시 절대 경로(/ 로 시작, .. 금지)만 허용됩니다.')
  .should('be.visible');

// 원격 전송 토글 (v-switch: :visible 필터 제외)
cy.get('input[aria-label="원격 전송(FTP/SFTP 등)"]')
  .should('exist')
  .and('have.attr', 'role', 'checkbox');

cy.contains('[보관>접속기록 보관]의 전송 설정(FTP/SFTP/SSH/S3)을 재사용하여 압축 아카이브를 원격지로 전송합니다')
  .should('be.visible');

// 접속기록 보관 설정 바로가기 버튼 (launch 아이콘)
cy.get('button.c-iab')
  .should('be.visible')
  .find('.material-icons')
  .should('contain.text', 'launch');
