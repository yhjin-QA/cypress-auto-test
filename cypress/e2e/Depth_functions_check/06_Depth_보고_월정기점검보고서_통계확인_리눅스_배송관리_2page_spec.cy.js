...
// 🌟 종료일자 달력에서도 '월(Month)'을 변경하는 과정을 수행합니다.
cy.get('.menuable__content__active').find('.v-date-picker-header__value button').click({ force: true });
cy.get('.v-date-picker-table--month').filter(':visible').contains(currentMonthText).click({ force: true });
...
cy.contains('div:visible', '전체:', { timeout: 30000 }).invoke('text')
  .then((text) => {
    ...
    cy.get('.menuable__content__active').filter(':visible').contains('.v-list__tile__title', '리눅스_배송관리').scrollIntoView().should('be.visible').click({ force: true });
    cy.wait(1000);
    cy.get('body').type('{esc}');
...
cy.get('.apexcharts-legend').eq(0).find('.apexcharts-legend-text').each(($legendInfo, index) => {
    ...
    // 기존 잔여정책이 존재한다면 삭제  
    // ==========================================
    // Depth통계확인_page2_auto 가 여러개 존재한다면 다 삭제하도록 코드-----------------------------------
    // 1. 반복 삭제를 수행할 함수를 정의합니다.
     const deleteAllReports = () => {
       // body 전체를 가져와서 동기적으로 검사합니다.
       cy.get('body').then(($body) => {
         // 만약 화면(행)에 'Depth통계확인_page2_auto'라는 글자가 1개라도 남아있다면?
         if ($body.find('tr:contains("Depth통계확인_page2_auto")').length > 0) {
      
           // --- [삭제 로직 시작] ---
           // 가장 위에 있는 'Depth통계확인_page2_auto' 행을 찾아서 휴지통 클릭
           cy.contains('tr', 'Depth통계확인_page2_auto').find('.fa-trash').closest('button').then(($btn) => {
           ...
           // 더 이상 'Depth통계확인_page2_auto'가 없다면 로그를 남기고 종료합니다.
            cy.log('모든 중복 Depth통계확인_page2_auto 삭제 완료!');
         }
       });
     };
      cy.wait(4000);
      ...
      cy.contains('a', 'Depth통계확인_page2_auto').should('not.exist');
      cy.wait(1000);
      //-----------------------------------------------------------------------------------
    ...
    // 보고서 추가화면에서 보고서이름 입력
    cy.get('input[aria-label="보고서 이름"]').filter(':visible').first().clear({ force: true }).type('Depth통계확인_page2_auto', { force: true });
    cy.wait(1000);
    // 보고서 추가화면에서 보고서설명 작성 
    cy.get('input[aria-label="설명"]').filter(':visible').first().clear({ force: true }).type('Page2 통계확인용 보고서입니다.', { force: true });
    cy.wait(1000);
...
cy.contains('a', 'Depth통계확인_page2_auto').click({ force: true });
cy.wait(1000);
