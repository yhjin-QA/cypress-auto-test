...
// -----------------------------------------------------------
    ...
    //예외처리  test_auto_비인가 접근 사용자 삭제 --------------------------
    // 1. [조건부 삭제] test_auto_비인가 접근 사용자 정책이 있으면 삭제, 없으면 패스
    cy.get('body').then(($body) => {
    // jQuery의 :contains 선택자를 이용해 해당 텍스트가 있는 <tr>을 찾습니다.
    const hasPolicy = $body.find('tr:contains("test_auto_비인가 접근 사용자")').length > 0;
...
// ================================================================
// [STEP 1] WAS 시스템 로그인 및 이상행위 - 기타휴직자 명슬혼(RxQVxZ23)
// ================================================================
...
cy.origin('http://10.10.54.22:8080', () => {
  ...
  cy.get('#id').should('be.visible').clear().type('RxQVxZ23');
...
cy.get('.menuable__content__active').filter(':visible').within(() => {
  ...
  cy.contains('.v-list__tile__title', '비인가 접근 사용자').scrollIntoView().should('be.visible').closest('.v-list__tile').click({ force: true });
});
...
cy.get('tbody', { timeout: 15000 }).contains('tr', '명슬혼(RxQVxZ23)').should('be.visible');
...
cy.get('tbody tr').filter(':visible').first().within(() => {
  ...
  // 2. 텍스트 검증
  cy.contains('명슬혼(RxQVxZ23)').should('be.visible');
 ...
 cy.log('🎉 분석 - 비인가 접근 -  기타 휴직 상태 사용자 확인 및 랜덤 경보등급 검증 완료!');
  ...
  cy.contains('button.has-child', '분석').click({ force: true });
  cy.wait(2000); // 메뉴 펼쳐짐 대기
  // 이상행위정책 > 비인가 접근 사용자 정책 선택 
  cy.contains('.v-chip__content', '비인가 접근 사용자').should('be.visible').click({ force: true });

  // 추가된 정책명 : test_auto_비인가 접근 사용자  다시 재클릭
...
// ==========================================
  // CASE3 비인가 접근 사용자 상태 - 휴가자  (배혜택 loginid208)
  // ==========================================
  // 분석 탭 메뉴로 다시 이동 
  cy.contains('button.has-child', '분석').click({ force: true });
  cy.wait(2000); // 메뉴 펼쳐짐 대기
...
cy.origin('http://10.10.54.22:8080', () => {
  ...
  cy.log('1️⃣ UI를 통해 완벽하게 로그인을 수행합니다.');
  cy.visit('/uat/uia/egovLoginUsr.do');
  cy.get('#id').should('be.visible').clear().type('loginid208');
  cy.get('#password').should('be.visible').clear().type('Manager1{enter}');
...
cy.get('tbody tr').filter(':visible').first().within(() => {
  ...
  // 2. 텍스트 검증
  cy.contains('배혜택(loginid208)').should('be.visible');
  cy.contains('비인가 접근 사용자').should('be.visible');
  cy.contains('test_auto_비인가 접근 사용자').should('be.visible');
  cy.contains('존재').should('be.visible');
  cy.contains('소명 대상').should('be.visible');
 ...
 cy.log('🎉 분석 - 비인가 접근 - 휴가자 사용자 확인 및 랜덤 경보등급 검증 완료!');


  // ==========================================
  // [FINAL] 테스트 종료 및 메뉴 닫기
  // ==========================================
  cy.log('🎉 Depth 분석- 비인가 접근 사용자 테스트 시나리오 성공적으로 완료!');
  cy.get('body').type('{esc}');
