...
describe('로그캐치 Depth 배포점검목록 동작 테스트', () => {
  ...
  it('14_Depth_관리자페이지 설정_관리자_계정관리 자동화 시나리오', () => {
...
// -----------------------------------------------------------
    ...
    //3.0.5.1191_r35135 가로 스크롤 영향으로 보이지않음. 존재로 대체 
    cy.contains('th', '소속된 관리자').should('exist');
    cy.contains('th', '읽기 전용').should('exist');
    });
    // 1번 추가 버튼 확인
  ...
  // ================================================================
    ...
    // 혹은 페이지 목록 25개로 변경하고 싶을 때 (파라미터 전달)
    cy.changePageRow('25');
    ...
    cy.log('🎉 테스트용 신규 계정(AutoUser) 삭제 성공!');
