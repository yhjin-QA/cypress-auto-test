...
describe('로그캐치 Depth 배포점검목록 동작 테스트', () => {
  ...
  it('08_Depth_분석_미등록 사용자 접속 자동화 시나리오', () => {
...
// -----------------------------------------------------------
    ...
    //예외처리  test_auto_미등록 사용자 접속 삭제 --------------------------
    // 1. [조건부 삭제] test_auto_미등록 사용자 접속 정책이 있으면 삭제, 없으면 패스
    cy.get('body').then(($body) => {
    // jQuery의 :contains 선택자를 이용해 해당 텍스트가 있는 <tr>을 찾습니다.
    const hasPolicy = $body.find('tr:contains("test_auto_미등록 사용자 접속")').length > 0;
    ...
    //미등록 사용자 접속 목록에 정책이 잘 추가되었는지 검증하는 코드 
...
cy.origin('http://10.10.54.31:8088', () => {
  ...
  // ID 입력
  // id가 username인 요소에 'logcatch'를 입력합니다.
  cy.get('#username').should('be.visible').type('logcatch');
  cy.wait(1000);
  
  // '로그인' 텍스트를 포함한 버튼을 찾아 클릭
  cy.contains('button', '로그인').should('be.visible').click();
  cy.wait(1500);

  // 3. HTML 버튼 클릭 (id 값인 #html_btn을 타겟팅)
  cy.log('2️⃣ Excel 버튼을 클릭합니다.');
  cy.get('#html_btn').should('be.visible').click({ force: true });
    
  // 엑셀 다운로드 또는 내부 처리 스크립트가 돌아갈 시간을 넉넉히 줍니다.
  cy.wait(1000); 
  cy.log('✅ HTML 버튼 클릭 완료!');

  // 출력결과창 닫기 
  // data-dismiss 속성이 modal인 close 클래스 버튼 클릭
  cy.get('button.close[data-dismiss="modal"]').should('be.visible').click();
  cy.wait(1500);
});
...
cy.get('.menuable__content__active').filter(':visible').within(() => {
  ...
  cy.contains('.v-list__tile__title', '미등록 사용자 접속').scrollIntoView().should('be.visible').closest('.v-list__tile').click({ force: true });
});
...
cy.get('tbody', { timeout: 15000 }).contains('tr', '(logcatch)').should('be.visible');
...
cy.get('tbody tr').filter(':visible').first().within(() => {
  ...
  // 2. 텍스트 검증
  cy.contains('(logcatch)').should('be.visible');
  cy.contains('미등록 사용자 접속').should('be.visible');
  cy.contains('test_auto_미등록 사용자 접속').should('be.visible');
...
cy.log('🎉 분석 이상행위  미등록 사용자 접속 확인 및 랜덤 등급 검증 완료!');


  // ==========================================
  // [FINAL] 테스트 종료 및 메뉴 닫기
  // ==========================================
  cy.log('🎉 Depth 분석 - 미등록 사용자 접속  테스트 시나리오 성공적으로 완료!');
  cy.get('body').type('{esc}');
