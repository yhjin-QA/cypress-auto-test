...
// -----------------------------------------------------------
   ...
   // 그룹별 클릭하는 코드 
    ...
    // 접근 제한 업무시스템 등록-----------------------------------------------------------------------------
    // 정책이름 입력하기
    cy.get('input[aria-label="정책명"]').filter(':visible').last().clear({ force: true }).type('test_리눅스_배송관리 업무시스템 제한', { force: true });
    ...
    cy.contains('tr', 'test_리눅스_배송관리 업무시스템 제한').scrollIntoView({ block: 'center' }) 
     ...
     // 접근 제한 업무시스템 등록-----------------------------------------------------------------------------
    // 정책이름 입력하기
    cy.get('input[aria-label="정책명"]').filter(':visible').last().clear({ force: true }).type('test_리눅스_VIP고객관리 제한', { force: true });
    cy.wait(500);
