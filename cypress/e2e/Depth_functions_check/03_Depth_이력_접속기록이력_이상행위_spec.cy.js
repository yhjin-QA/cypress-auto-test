...
//depth
    ...
    cy.wait(500);
    // 데이터 이력있을시
    //cy.contains('tr', '비인가 IP 접근').should('contain', 'test_auto_비인가 IP 접근속') .find('i.g-ICriticalAlert').should('be.visible').and('have.css', 'color', 'rgb(169, 209, 142)');
    // 데이터 이력없을시
    // 현재 화면의 메인 컨텐츠 영역 안에 있는 'No data available'만 체크
    cy.get('main').contains('td.text-xs-center', 'No data available').should('exist');
    ...
    cy.wait(500);
    // 데이터 이력있을시
    //cy.contains('tr', '사전 소명 메뉴 접근').should('contain', 'test_auto_사전 소명 메뉴 접근') .find('i.g-ICriticalAlert').should('be.visible').and('have.css', 'color', 'rgb(169, 209, 142)');
    // 데이터 이력없을시
    // 현재 화면의 메인 컨텐츠 영역 안에 있는 'No data available'만 체크
    cy.get('main').contains('td.text-xs-center', 'No data available').should('exist');
    ...
    cy.wait(500);
    // 데이터 이력있을시
    cy.contains('tr', '개인정보 유형 과다사용').should('contain', 'test_auto_개인정보 유형 과다사용') .find('i.g-ICriticalAlert').should('be.visible').and('have.css', 'color', 'rgb(169, 209, 142)');
    // 데이터 이력없을시
    // 현재 화면의 메인 컨텐츠 영역 안에 있는 'No data available'만 체크
    //cy.get('main').contains('td.text-xs-center', 'No data available').should('exist');
