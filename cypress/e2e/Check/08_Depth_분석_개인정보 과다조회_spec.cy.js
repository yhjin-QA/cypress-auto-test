...
// -----------------------------------------------------------
    ...
    // 정책이름 입력 
    cy.get('input[aria-label="정책명"]').filter(':visible').clear({ force: true }).type('test_auto_개인정보과다조회', { force: true });
...
cy.origin('http://10.10.54.22:8080', () => {
  ...
  // 2. WAS 화면 UI 로그인 진행 (loginid208 계정)
...
// 사용자 검색 - 배혜택(loginid208)
// 1. 콤보박스에 검색어 입력
cy.get('input[aria-label="사용자"]').filter(':visible').clear({ force: true }).type('loginid208', { force: true });
cy.wait(1000); 
// 검색된 콤보박스 리스트  선택하기
cy.contains('.v-list__tile__title', 'loginid208').should('be.visible').click({ force: true });
cy.wait(1000);
// 선택 후 메뉴 닫기
cy.get('body').type('{esc}');
