...
// -----------------------------------------------------------
    ...
    //예외처리  test_auto_장기 미접속 사용자 삭제 --------------------------
    // 1. [조건부 삭제] test_auto_장기 미접속 사용자 정책이 있으면 삭제, 없으면 패스
    cy.get('body').then(($body) => {
    // jQuery의 :contains 선택자를 이용해 해당 텍스트가 있는 <tr>을 찾습니다.
    const hasPolicy = $body.find('tr:contains("test_auto_장기 미접속 사용자")').length > 0;
    ...
    // 정책이름 입력 
    cy.get('input[aria-label="정책 이름"]').filter(':visible').first().clear({ force: true }).type('test_auto_장기 미접속 사용자', { force: true });
    ...
    cy.wait(1000);
    // 그룹별중 총무,인사팀 클릭하는 코드
    cy.get('.v-menu__content').filter(':visible').contains('경영지원팀').click({ force: true });
    cy.wait(500);
    // 선택 후 메뉴 닫기
...
cy.get('.menuable__content__active').filter(':visible').within(() => {
  ...
  // 2. 그 활성 팝업창 안에서 '장기 미접속 사용자'를 선택
  cy.contains('.v-list__tile__title', '장기 미접속 사용자').scrollIntoView().should('be.visible').closest('.v-list__tile').click({ force: true });
});
...
cy.get('tbody', { timeout: 15000 }).contains('tr', '변잔수(KtqeIB85)').should('be.visible');
...
cy.get('tbody tr').filter(':visible').first().within(() => {
  ...
  // 2. 텍스트 검증
  cy.contains('변잔수(KtqeIB85)').should('be.visible');
  cy.contains('장기 미접속 사용자').should('be.visible');
  cy.contains('test_auto_장기 미접속 사용자').should('be.visible');
...
cy.log('🎉 분석 장기 미접속 사용자 확인 및 랜덤 등급 검증 완료!');


  // ==========================================
  // [FINAL] 테스트 종료 및 메뉴 닫기
  // ==========================================
  cy.log('🎉 Depth 분석 - 장기 미접속 사용자 테스트 시나리오 성공적으로 완료!');
  cy.get('body').type('{esc}');
