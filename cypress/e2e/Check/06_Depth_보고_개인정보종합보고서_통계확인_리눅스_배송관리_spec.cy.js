        ...
        cy.get('@dynamicChartData3').then((chartData3) => {
            ...
            cy.get('iframe', { timeout: 15000 })
              ...
              .within(() => {
                  ...
                  // =====================================================
                  // 📄 [14페이지 진입] 2번 차트 검증 (이상행위 유형별 현황)
                  // =====================================================
                  cy.log('➡️ [14페이지] 2번 차트 검증을 위해 이동합니다. (8번 클릭)');
                  ...
                  cy.root().invoke('text').then((bodyText14) => {
                      ...
                      Object.keys(chartData2).forEach((typeName) => {
                          ...
                          if (match && match[1]) {
                              ...
                              throw new Error(`❌ 14페이지에서 2번 차트의 "${typeName}" 항목을 찾을 수 없습니다.`);
                          }
                      });
                  });

                  cy.log('🎉 [최종 통과] 3번(5p) -> 1번(6p) -> 2번(14p) 차트의 모든 보고서 데이터 정합성 검증이 완벽하게 완료되었습니다!');
              }); // end within
        });
    });
});
