...
// -----------------------------------------------------------
   ...
   ///////////////////////////////////////////////
    ...
    // 정보 사용자 팝업창에서 이름 검색 - 진윤호 입력 
    cy.get('.v-dialog').filter(':visible').find('input[aria-label="사용자"]').type('진윤호', { force: true });
    cy.wait(500);

    // 정보사용자 팝업창에서 진윤호이라는 사람 그옆 체크박스 클릭
    cy.contains('tr', '진윤호').find('.v-icon').click({ force: true });
    cy.wait(500);

    // 체크가 잘되어있는지 검증코드
    cy.contains('tr', '진윤호').find('.v-icon').should('contain', 'check_box');
