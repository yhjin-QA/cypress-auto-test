describe('시나리오 6: 고객 보안 제어 및 감사 로그(Modal) 자동화 검증', () => {
  ...
  it('특정 고객에 대한 민감한 보안 제어(마스킹 해제, 파기 등) 시 모달 경고와 로깅이 정상 작동한다', () => {
    ...
    // ---------------------------------------------------------
    // 🎯 3단계: 대량 고객 정보 PDF 리포트 다운로드 타격 (NEW)
    // ---------------------------------------------------------
    cy.log('▶ 3단계: 대량 고객 정보 PDF 다운로드 로깅 타격');
    
    // 브라우저의 window.print() 창이 뜨면 테스트 봇이 멈추는 현상을 방지하기 위해 stub(가로채기) 처리
    cy.window().then((win) => {
      cy.stub(win, 'print').as('printStub');
    });

    // PDF 다운로드 버튼 클릭
    cy.get('#btn_pdf_download').click();

    // 브라우저의 인쇄 기능(PDF 저장)이 정상적으로 호출되었는지 스파이 검증
    cy.get('@printStub').should('be.calledOnce');
    cy.log('✅ 대량 데이터 PDF 추출 차단/로깅 시뮬레이션 성공!');
  });
});