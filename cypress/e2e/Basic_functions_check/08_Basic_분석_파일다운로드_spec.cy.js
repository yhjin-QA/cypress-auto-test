...
// -----------------------------------------------------------
    ...
    // 파일다운로드 정책 추가화면 진입----------------------------------------
    // 정책이름 입력 
    cy.get('input[aria-label="정책명"]').filter(':visible').clear({ force: true }).type('test_auto_파일다운로드', { force: true });
    ...
    cy.contains('tr', 'test_auto_파일다운로드').find('td').contains('false').should('be.visible');
    cy.wait(1000);
     //---------------------------------------------------------------------------------------
    ...
    // ==========================================
    cy.log('🎉 분석 - 파일다운로드 테스트 시나리오 성공적으로 완료!');
    cy.get('body').type('{esc}');
