...
// -----------------------------------------------------------
    ...
    cy.get('body').type('{esc}');
     ...
     // 개인정보 유형 선택하기
     cy.get('.v-menu__content').filter(':visible').as('dropdown');

// 목록을 여러 단계로 스크롤하면서 "휴대전화번호" 항목이 나타날 때까지 탐색-----------------------------------
function scrollAndFind(maxAttempts = 15) {
  let attempt = 0;

  function tryFind() {
    return cy.get('@dropdown').then(($menu) => {
      const $target = $menu.find('.v-list__tile__title').filter((i, el) => {
        return Cypress.$(el).text().trim() === '휴대전화번호';
      });
      ...
      if (attempt >= maxAttempts) {
        throw new Error('휴대전화번호 항목을 찾지 못했습니다 (스크롤 한계 도달)');
      }

      attempt++;
      // 드롭다운 컨테이너 자체를 스크롤 (내부 스크롤 대상 요소 확인 필요)
      cy.get('@dropdown').scrollTo('bottom', { ensureScrollable: false, duration: 200 });
      cy.wait(200);
      tryFind();
    });
  }
