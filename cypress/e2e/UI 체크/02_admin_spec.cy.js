...
/**코드 시작  */
describe('로그캐치 사이트 테스트(Admin_Mode)', () => {
  ...
  Cypress.on('uncaught:exception', (err, runnable) => {
    ...
    const ignoredErrors = [
      ...
      'Loading chunk', //  [추가됨] 이 에러 무시 설정 추가
      'operate.task.packageManagement' //  [추가됨] 특정 파일명도 추가하면 더 안전함
    ];
  ...
  it('로그캐치 UI기본체크(admin)', () => {
   ...
   // -----------------------------------------------------------
   cy.wait(1000); // 팝업이 뜨는 찰나의 시간을 기다려줍니다.
   cy.get('body').then(($body) => {
    
    // 2. jQuery 문법(.find)으로 해당 요소가 있는지 '길이(length)'로 체크합니다.
    // 주의: 여기서는 cy.contains를 쓰면 안 됩니다!
    if ($body.find('.v-card__title:contains("이미 접속 중인 계정입니다.")').length > 0) {
...
/*
   ...
   // 설명: 'c-headline' 클래스를 가진 요소 중에 '이상행위' 글자가 보여야 한다.
    ...
    // STEP 4: 현황서브메뉴 
    // ==========================================
    cy.contains('button', '현황').should('be.visible').click({ force: true });
    cy.wait(1000); // 서브 메뉴가 펼쳐질 시간 대기
    ...
    cy.contains('.c-headline', '검색 조건').should('exist');
    cy.get('.v-btn__content').filter(':visible').contains('검색').should('be.visible');
    cy.get('label').filter(':visible').contains(/^\s*기간\s*$/).should('be.visible');
    ...
    // STEP 5: 소명 서브메뉴 
    // ==========================================
    cy.contains('button', '소명').should('be.visible').click({ force: true });
    cy.wait(1000); // 서브 메뉴가 펼쳐질 시간 대기
    ...
    // STEP 6: 자산 서브메뉴 
    // ==========================================
    cy.contains('button', '자산').should('be.visible').click({ force: true });
    cy.wait(3000);
    ...
    // STEP 7: 보고 서브메뉴 
    // ==========================================
    cy.contains('button', '보고').should('be.visible').click({ force: true });
    cy.wait(2000);
    cy.log('--- 화면 검증 시작 ---');
    ...
    // STEP 9: 분석 서브메뉴 
    // ==========================================
    cy.contains('button.has-child', '분석').should('be.visible').click({ force: true });
   ...
   // 2. [핵심] 알림창이 떴는지 확인하고 처리 (If문)
    ...
    // ==========================================
    cy.log('🚀 점검 탭 클릭');
    cy.contains('button', '점검').should('be.visible').click({ force: true });
    cy.wait(2000);
    cy.log('--- 화면 검증 시작 ---');
    ...
    */
    ...
    cy.contains('button.side-menu span', /^\s*관리\s*$/).closest('button').click({ force: true });
    cy.wait(3000);
    cy.log('--- 서브메뉴 [클러스터링 서버 관리] 클릭 ---');
    cy.contains('.v-list__tile__title', /^\s*클러스터링 서버 관리\s*$/).should('be.visible').click({ force: true });
    cy.wait(4000); // 화면 전환 대기
